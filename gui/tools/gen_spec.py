...
def emit_option(command, action, out):
    ...
    out.append("    { %s, %s, %s, Kind::%s, %s, %s, %s, %s, %s, { %s } }," % (
        cstring(action.dest),
        ...
        "true" if action.nargs in ("*", "+") else "false",
...
def repoint_actions():
    sys.path.insert(0, os.path.join(CONVERTER, "tools"))
    import repoint_package
    return real_actions(repoint_package.build_parser())
...
def main():
    ...
    lines.append("inline const Opt kRepointOptions[] = {")
    for action in repoint_actions():
        emit_option("repoint", action, lines)
    lines.append("};")
    lines.append("")
    ...
    lines.append("inline constexpr int kRepointOptionCount = %d;"
                 % len(repoint_actions()))
